...
# Scriptet kan ligge enten i repo-rot eller i docs/. Workflowen under legger det i docs/.
SCRIPT_DIR = Path(__file__).resolve().parent
if SCRIPT_DIR.name == 'docs':
    DOCS_DIR = SCRIPT_DIR
elif (SCRIPT_DIR / 'docs').is_dir():
    DOCS_DIR = SCRIPT_DIR / 'docs'
elif (SCRIPT_DIR.parent / 'docs').is_dir():
...
def find_js_function_span(html, signature):
    """Returnerer (start, end) for en JS-funksjon ved å telle klammer.

    signature kan f.eks. være 'async function updateFylkeFromPosition'.
    Dette er mer robust enn eksakt strengmatch mot minifisert HTML.
    """
...
def replace_js_function(html, signature, replacement, critical_name):
    span = find_js_function_span(html, signature)
    if not span:
        miss(f"Fant ikke JS-funksjonen {critical_name}; patchen ville ellers gitt grønn workflow uten reell endring.")
        return html
...
def patch_idb_version(html):
    '''Øk IDB_VER til minst 4 og legg til error_log/progress_snapshots uten å kreve eksakt streng.'''
    m = re.search(r"const\s+IDB_NAME\s*=\s*'nvdb_tiles'[^;]*IDB_VER\s*=\s*(\d+)\s*;", html)
    if not m:
        miss("Fant ikke IDB-konstantlinjen. Kan ikke opprette error_log-store.")
        return html
    line = m.group(0)
    new_line = line
    inserts = []
    if 'IDB_PROGRESS_SNAPSHOT_STORE' not in new_line:
        inserts.append("IDB_PROGRESS_SNAPSHOT_STORE='progress_snapshots'")
    if 'IDB_ERROR_LOG_STORE' not in new_line:
        inserts.append("IDB_ERROR_LOG_STORE='error_log'")
    if inserts:
        new_line = re.sub(r",\s*IDB_VER\s*=", ',' + ','.join(inserts) + ',IDB_VER=', new_line, count=1)
    ver_m = re.search(r"IDB_VER\s*=\s*(\d+)", new_line)
    if ver_m and int(ver_m.group(1)) < 4:
        new_line = re.sub(r"IDB_VER\s*=\s*\d+", "IDB_VER=4", new_line, count=1)
...
def patch_open_idb(html):
    '''Utvid openIDB() til å opprette error_log store ved upgrade, mer robust enn eksakt strengmatch.'''
    if "createObjectStore(IDB_ERROR_LOG_STORE" in html:
        print("[INFO] openIDB() har allerede error_log-store")
        return html
    span = find_js_function_span(html, "function openIDB")
    if not span:
        miss("openIDB() ble ikke funnet. IndexedDB-upgrade blir ikke patchet.")
        return html
    fn = html[span[0]:span[1]]
    if "req.onupgradeneeded" not in fn:
        miss("openIDB() mangler req.onupgradeneeded. Kan ikke legge til object store trygt.")
        return html
    progress_line = "if(!db.objectStoreNames.contains(IDB_PROGRESS_SNAPSHOT_STORE))db.createObjectStore(IDB_PROGRESS_SNAPSHOT_STORE,{keyPath:'ts'});"
    error_line = "if(!db.objectStoreNames.contains(IDB_ERROR_LOG_STORE))db.createObjectStore(IDB_ERROR_LOG_STORE,{keyPath:'id',autoIncrement:true});"
    insert_lines = ""
    if progress_line not in fn:
        insert_lines += progress_line
    insert_lines += error_line
    # Sett inn rett før upgrade-callbacken avsluttes; tåler både minifisert og formatert JS.
    m_end = re.search(r"\}\s*;\s*req\.onsuccess", fn)
    if not m_end:
        m_end = re.search(r"\}\s*;\s*req\.onerror", fn)
    if not m_end:
        miss("Fant ikke slutten av req.onupgradeneeded i openIDB().")
        return html
    idx = m_end.start()
    fn = fn[:idx] + insert_lines + fn[idx:]
    return html[:span[0]] + fn + html[span[1]:]



def inject_error_log_module(html):
    '''Sett inn error-log-modul rett etter openIDB().'''
    if 'function logError(' in html:
        print('[INFO] Error-log-modul finnes allerede')
        return html
    error_log_module = r'''
...
// Samlet best-effort persist. Brukes ved fylkesbytte og pagehide.
async function forcePersistProgressNow(source){
 let ok=false;
 try{if(typeof flushAllPendingWritesNow==='function'){await flushAllPendingWritesNow();ok=true}}catch(e){logError('error',source||'persist','flushAllPendingWritesNow feilet',{err:String(e)})}
 try{if(typeof flushProgressPersistence==='function'){await flushProgressPersistence();ok=true}}catch(e){logError('error',source||'persist','flushProgressPersistence feilet',{err:String(e)})}
 try{if(typeof saveProgressSnapshotToIDB==='function'){await saveProgressSnapshotToIDB();ok=true}}catch(e){logError('error',source||'persist','saveProgressSnapshotToIDB feilet',{err:String(e)})}
 try{if(typeof saveGeomCache==='function')saveGeomCache()}catch(e){logError('warn',source||'persist','saveGeomCache feilet',{err:String(e)})}
 try{await flushErrorLogToIDB()}catch(e){}
 if(ok)markSaveSuccess(source||'persist');
 return ok;
}
// Flush ved app-bytte slik at vi ikke mister oppføringer
window.addEventListener('pagehide',()=>{try{forcePersistProgressNow('pagehide')}catch(e){try{flushErrorLogToIDB()}catch(_){}}});
...
function markSaveSuccess(source){
 ...
 if(typeof gps!=='undefined'&&gps.watching)try{updateSaveStatusBadge()}catch(e){}
}
...
setInterval(()=>{if(typeof gps!=='undefined'&&gps.watching)try{updateSaveStatusBadge()}catch(e){}},5000);

'''
    # Sett inn rett etter openIDB-funksjonen. Tidligere brukte scriptet tileKey som marker,
    # men det var sårbart for små endringer i minifisert baseline.
    span = find_js_function_span(html, 'function openIDB')
    if span:
        return html[:span[1]] + error_log_module + html[span[1]:]
    marker = "function tileKey(lat,lon,r,f){"
    if marker in html:
        return html.replace(marker, error_log_module + marker, 1)
    miss("Fant verken openIDB() eller tileKey-marker; error-log-modulen ble ikke injisert.")
    return html
...
def patch_save_progress_snapshot(html):
    ...
    replacement = (
        ...
        "  if(typeof markSaveSuccess==='function')markSaveSuccess('progress-snapshot');\n"
        "  return true;\n"
        " }catch(e){\n"
        "  if(typeof markSaveError==='function')markSaveError('progress-snapshot',e);\n"
        "  else console.warn('Kunne ikke lagre samlet fremdrift i IndexedDB',e);\n"
        "  return false;\n"
        " }\n"
        "}"
    )
    return replace_js_function(html, "async function saveProgressSnapshotToIDB", replacement, "saveProgressSnapshotToIDB")
...
def patch_update_fylke(html):
    ...
    replacement = (
        ...
        "let f;try{f=await detectFylkeFromAPI(lat,lon)}catch(e){if(typeof logError==='function')logError('warn','fylke-detect','Geonorge feilet',{err:String(e),lat,lon});return}"
        "if(!f)return;"
        "const prevFylke=gps.detectedFylke;"
        "if(prevFylke&&prevFylke!==f.id){"
        " if(typeof logError==='function')logError('info','fylke-bytte','Krysset fylkesgrense, lagrer før fylke skiftes',{fra:prevFylke,til:f.id,lat,lon});"
        " try{if(typeof forcePersistProgressNow==='function')await forcePersistProgressNow('fylke-bytte-før');else if(typeof saveProgressSnapshotToIDB==='function')await saveProgressSnapshotToIDB()}"
        " catch(e){if(typeof logError==='function')logError('error','fylke-bytte','Lagring før fylke-bytte feilet',{err:String(e)})}"
        "}"
        "gps.detectedFylke=f.id;"
        "if(sel&&sel.dataset.manual!=='1'&&sel.value)sel.value='';"
        "updateHeaderTitle();"
        "if(prevFylke&&prevFylke!==f.id){"
        " try{if(typeof forcePersistProgressNow==='function')await forcePersistProgressNow('fylke-bytte-etter');else if(typeof saveProgressSnapshotToIDB==='function')await saveProgressSnapshotToIDB()}"
        " catch(e){if(typeof logError==='function')logError('error','fylke-bytte','Lagring etter fylke-bytte feilet',{err:String(e)})}"
        "}"
        "}"
    )
    return replace_js_function(html, "async function updateFylkeFromPosition", replacement, "updateFylkeFromPosition")



def patch_gps_ui_save_status(html):
    '''Legg til save-status-badge i GPS-UI. Tåler litt ulik whitespace i template.'''
    if 'id="gps-save-status"' in html:
        return html
    badge = '<span class="gps-save-status" id="gps-save-status" style="font-size:10px;font-weight:900;margin-left:6px;font-variant-numeric:tabular-nums"></span>'
    pat = re.compile(r'(<span\s+class="gps-heading"\s+id="gps-heading"[^>]*>.*?<span\s+class="heading-dir"\s+id="gps-heading-dir"></span>\s*</span>)', re.DOTALL)
    m = pat.search(html)
    if m:
        return html[:m.end()] + badge + html[m.end():]
    warn("GPS-UI save-status badge ble ikke injisert — fant ikke gps-heading-blokken")
    return html
...
def main():
    ...
    steps = [
        ("Versjon oppdatert", patch_version),
        ("IDB-versjon minst 4 + error_log/progress_snapshots", patch_idb_version),
        ("openIDB() utvidet med error_log store", patch_open_idb),
        ("Error-log-modul injisert", inject_error_log_module),
        ("saveProgressSnapshotToIDB wrappet med try/catch + markSaveSuccess", patch_save_progress_snapshot),
        ("updateFylkeFromPosition får synkron flush ved fylke-bytte", patch_update_fylke),
        ("GPS-UI har save-status-badge", patch_gps_ui_save_status),
        ("renderGPS oppdaterer save-status-badge", patch_render_gps_save_status),
        ("Meny ryddet (v2.0.51) + ny feillogg-knapp", patch_menu_v2051),
        ("Event-listenere for feillogg lagt til", patch_event_listeners),
        ("Fremdrift-fanen redigeringsliste injisert (v2.0.50)", patch_progress_split_view),
        ("Restoresnap-knapp wiret opp", patch_restoresnap_button),
        ("Repairdir-knapp wiret opp", patch_repairdir_button),
    ]
    for label, fn in steps:
        prev = html
        html = fn(html)
        changed = "endret" if html != prev else "uendret"
        print(f"[OK] {label} ({changed})")

    if CRITICAL_MISSES:
        print("\n=== KRITISKE PATCHER MANGLER ===")
        for msg in CRITICAL_MISSES:
            print(f" - {msg}")
        print("\nAvbryter slik at workflow ikke ser grønn ut når index.html ikke faktisk er patchet.")
        sys.exit(1)
    ...
    if html == before:
        print("[ERROR] Ingen endringer ble gjort i HTML. Avbryter.")
        sys.exit(1)
